...
def test_real_mode_missing_playbook_file_fails_cleanly(monkeypatch, tmp_path):
    # Empty directory — no .yml files — so the playbook genuinely doesn't exist.
...
def test_real_mode_failure_includes_stderr(monkeypatch, tmp_path):
    # This is the scenario that matters most in practice: a node is
    # unreachable, ansible-playbook exits non-zero, and the error needs
    # to actually reach the dashboard instead of getting swallowed.
    monkeypatch.setattr(runner, "DEMO_MODE", False)
    playbook_dir = tmp_path / "playbooks"
    playbook_dir.mkdir()
...
def test_real_mode_calls_ansible_playbook_with_correct_args(monkeypatch, tmp_path):
    # Guards against a future refactor accidentally dropping the inventory
    # flag or pointing at the wrong playbook path.
    monkeypatch.setattr(runner, "DEMO_MODE", False)
    playbook_dir = tmp_path / "playbooks"
    playbook_dir.mkdir()
