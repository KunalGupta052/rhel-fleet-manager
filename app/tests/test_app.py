...
Uses the create_app() factory with an in-memory SQLite database (see
conftest.py), so nothing here touches the real fleet.db. run_playbook is
mocked in every test that triggers it, so tests never shell out to Ansible
or need real SSH-reachable nodes.
"""
